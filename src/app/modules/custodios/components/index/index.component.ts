...
import { CustodiansStore } from 'src/app/features/custodians/store/custodians.store';
...
export class IndexComponent implements OnInit {
  store  = inject(CustodiansStore);
  dialog = inject(MatDialog);

  displayedColumns = ['numero','nombre','cedula','Cargo','Departamento','borrado','editar'];
  ...
  // Departamentos placeholder (aún referenciado en template de algunos custodios)
  departamentos: any[] = [];

  ngOnInit(): void { this.store.load(); }

  openDialog(enter?: string, exit?: string): void {
    this.dialog.open(FormComponent, { width: 'auto', height: 'auto', data: 'probando dialogo' });
  }
