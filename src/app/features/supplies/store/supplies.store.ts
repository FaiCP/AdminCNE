...
import { SuppliesRepository } from '../data-access/supplies.repository';

@Injectable({ providedIn: 'root' })
export class SuppliesStore {
  private repo   = inject(SuppliesRepository);
  ...
  saveEdit(): void {
    ...
    if (id === null) return;
    this.repo.updateSupply(id, this._editingSnap()).subscribe({
      next: () => {
  ...
  delete(id: number): void {
    ...
    this.repo.deleteSupply([id]).subscribe({
      next: () => {
